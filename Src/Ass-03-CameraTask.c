  ...
  while (FrameCounter < 20)
  {
    ...
    if (LineCounter < XSIZE/2)
    {
      ...
      if (osMutexWait (LCDMutexHandle, 0) == osOK)
      {
        ...
        ili9325_SetDisplayWindow(XSIZE-YSIZE/2-1-4, 1+4, YSIZE/2, XSIZE/2-4);
        ...
        // HAL_DMA_Start (&hdma_memtomem_dma2_stream0, (uint32_t *)camera_buffer, *(uint16_t *)LCD_RAM, XSIZE);
